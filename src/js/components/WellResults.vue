...
<!--        </div>-->
            ...
            <div class="flex flex-2" v-if="wellAnalyzer.manyDefectWells().length">
                <article>
                    <table>
                        ...
                        <tbody>
                        <tr v-for="(well, i) in wellAnalyzer.manyDefectWells()">
                            <td>{{ well.name }}</td>
                            <td>{{ well.defects }}</td>
                        </tr>
                        </tbody>
                        <tfoot>
                        <tr>
                            <td colspan="2">The following wells are not qualified for Matrix Acidizing and were taking
        ...
        <div class="flex flex-2">
            <article>
                ...
                <table>
                    ...
                    <tbody>
                    <tr v-for="(well, i) in wellAnalyzer.wellRank()">
        ...
        <div class="flex flex-2">
            <article>
                ...
                <table v-if="wellAnalyzer.easyWorkOver().length">
                    ...
                    <tbody>
                    <tr v-for="well in wellAnalyzer.easyWorkOver()">
                        <td>{{ well.name }}</td>
                        <td>{{ well.cof }}</td>
                    </tr>
                    </tbody>
                    <tfoot>
                    <tr>
                        <td colspan="2">These wells have Easy work over complexity Coiled Tubing.</td>
                    </tr>
                    </tfoot>
                </table>
                <p v-else>No Wells Present Here.</p>
            ...
            <article>
                ...
                <table v-if="wellAnalyzer.diffWorkOver().length">
                    ...
                    <tbody>
                    <tr v-for="well in wellAnalyzer.diffWorkOver()">
                        <td>{{ well.name }}</td>
                        <td>{{ well.cof }}</td>
                    </tr>
                    </tbody>
        ...
        <table>
            ...
            <tr>
                <th>Indicators</th>
                <th v-for="well in wellAnalyzer.wellRank()">{{ well.name }}</th>
            </tr>
            </thead>
            <tbody>
            <tr v-for="param in econsParams">
                <td>{{ param.textName }}</td>
                <td v-for="well in wellAnalyzer.wells">{{ well[param.varName] }}</td>
            </tr>
            </tbody>
        </table>
        <br/>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            wellAnalyzer() {
                return new WellAnalyzer(store.getters.wells);
